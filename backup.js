...
function gsSectionHTML(SC) {
  ...
  if (gs) return `
    ...
    <button class="btn block ghost" style="margin-top:8px" data-act="gsother">Cambiar conexión o clave</button>
...
const ACTS = {
  ...
  async del() {
    const rec = await Store.get(S.folder, S.id);
    const SC = SCHEMAS[S.folder];
    const sincronizada = !!(SC.def && SC.def.gs);
    if (sincronizada && rec) await tombAdd(S.folder, rec.data);
    await Store.remove(S.folder, S.id);
    await Store.delDraft(draftKey());
    S.sheet = null;
    toast(sincronizada ? 'Eliminado. En la próxima sincronización se borra también en la planilla y en los demás teléfonos.' : 'Registro eliminado');
    await goList();
  },
  ...
  async gsother() {
    const cur = (SCHEMAS[S.folder].def && SCHEMAS[S.folder].def.gs) || (await Store.setting('gsScript')) || {};
    openSheet(sheetHTML('Conectar con Google Sheets',
      `<p class="note" style="margin-bottom:8px"><b>Método gratis</b> (sin tarjeta, sin Google Cloud): creá una planilla, pegale el script <b>GOOGLE_SCRIPT.gs</b>, publicalo y pegá acá la URL y tu clave. Si cambiaste el código del script, acordate: Implementar → Administrar implementaciones → ✏ → <b>Nueva versión</b>.</p>
       <div class="fhint" style="margin-bottom:4px">URL del <b>script</b> (script.google.com/…/exec) — NO la de la planilla. Es una sola para todo el servicio, la misma en todas las carpetas.</div>
       <input type="text" id="gsscript" placeholder="https://script.google.com/macros/s/…/exec" value="${esc(cur.script || cur.url || '')}" style="margin-bottom:8px">
       <input type="text" id="gstoken" placeholder="Clave (la del script)" value="${esc(cur.token || '')}" style="margin-bottom:8px">
       <button class="btn primary block" data-act="gsscriptgo">Vincular / Probar de nuevo</button>
       ${SCHEMAS[S.folder].def && SCHEMAS[S.folder].def.gs ? '<button class="btn block danger" style="margin-top:8px" data-act="gsunlink">Desvincular esta carpeta</button>' : ''}
  ...
  async gsscriptgo() {
    ...
    if (/docs\.google\.com\/spreadsheets/.test(url))
      return toast('Esa es la URL de la PLANILLA. Acá va la del SCRIPT: en Apps Script → Implementar → Administrar implementaciones → copiá la URL que termina en /exec.', 'err');
    if (!/^https:\/\/script\.google\.com\/.+\/exec$/.test(url))
      return toast('Esa URL no es la del script publicado. En Apps Script: Implementar → Administrar implementaciones → copiá la URL que termina en /exec.', 'err');
    ...
    try {
      ...
      openSheet(sheetHTML('Sincronizado',
        `<div class="stats" style="margin-top:4px">
          ...
          ${r.borrados ? `<div class="stat"><div class="k">Borrados</div><div class="v">${r.borrados}</div></div>` : `<div class="stat"><div class="k">Conflictos</div><div class="v">${r.conflictos.length}</div></div>`}
        </div>
