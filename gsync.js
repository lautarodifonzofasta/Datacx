...
const Store = {
  ...
  /* Marca todos los registros de la carpeta como sincronizados AHORA, sin tocar
     updatedAt: esa diferencia es la que distingue "lo edité después" de "no". */
  async markSynced(folder, ts) {
    const rs = await this.all(folder);
    for (const r of rs) {
      r.synced = true; r.syncedAt = ts;
      if (isSys(folder)) await db[folder].put(r); else await db.records.put(r);
    }
  },
  async count(folder) {
